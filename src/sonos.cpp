// modified from https://github.com/piis3/sonos-buttons
...
static const char *PLAYER_SEARCH =
    "M-SEARCH * HTTP/1.1\r\n"
...
static const std::string GET_VOLUME_CALL =
    "<?xml version=\"1.0\"?>"
...
static std::string changeVolumeCall(int volume) {
    return "<?xml version=\"1.0\"?>"
           ...
           "<Channel>Master</Channel>"
           "<DesiredVolume>" +
           std::string(String(volume).c_str()) +
           "</DesiredVolume>"
           "</u:GetVolume>"
           "</s:Body>"
           "</s:Envelope>";
...
std::string tagValue(std::string xmlData, std::string tagName) {
    ...
    ParseState state = {captured, false, tagName};

    XML_StartElementHandler start = [](void *myState, const char *el,
                                       const char **attr) {
    ...
    XML_CharacterDataHandler charData = [](void *myState, const char *s,
                                           int len) {
        ParseState *d = (ParseState *)myState;
        if (d->capturing) {
...
std::string filterDeviceLocation(std::string xmlData, std::string targetUid) {
    ...
    ParseState state = {location, targetUid};
...
/**
 * Given any sonos' address, ask it for the zone topology to find the right
 * sonos
 */
IPAddress zoneTopology(HTTPClient *http, std::string host,
                       std::string targetUid) {
    ...
    if (http->begin(host.c_str(), SONOS_PORT, "/ZoneGroupTopology/Control")) {
        ...
        if (httpCode == 200) {
            // The body here is an XML doc embedded in the body of another, so
            // just pull out the first one, then run it through the next parser
            std::string innerXml = tagValue(
                std::string(http->getString().c_str()), "ZoneGroupState");
...
IPAddress discoverSonos(std::string uid) {
    ...
    if (udp.listenMulticast(IPAddress(239, 255, 255, 250), 1900)) {
        ...
        if (foundAddr) {
            ...
            // Now we need to ask whatever sonos we found about the topology to
            // find what we care about
            HTTPClient http;
            http.setConnectTimeout(HTTP_TIMEOUT);
            http.setTimeout(HTTP_TIMEOUT);

            IPAddress ourSonos = zoneTopology(
                &http, std::string(foundAddr.toString().c_str()), uid);
            if (ourSonos) {
...
int sonosOperation(int (*operation)(HTTPClient *http, IPAddress target),
                   IPAddress targetSonos) {
    HTTPClient http;
    http.setReuse(false);
    http.setConnectTimeout(HTTP_TIMEOUT);
    ...
    if (errorCode) {
        // I think we just care about noticing that we might have to rediscover
        // the sonos but punt for now
...
std::string playState(HTTPClient *http, IPAddress targetSonos) {
    ...
    if (http->begin(targetSonos.toString(), SONOS_PORT,
        ...
        if (httpCode == 200) {
             ...
             */
            return tagValue(std::string(http->getString().c_str()),
...
int sonosPlay(HTTPClient *http, IPAddress targetSonos) {
    ...
    if (http->begin(targetSonos.toString(), SONOS_PORT,
        ...
        http->addHeader(
            "SOAPACTION",
            ("urn:schemas-upnp-org:service:AVTransport:1#" + requestState)
                .c_str());
...
int getVolume(HTTPClient *http, IPAddress targetSonos) {
    if (http->begin(targetSonos.toString(), SONOS_PORT,
        ...
        if (httpCode != 200) {
            ...
            auto volStr = tagValue(std::string(http->getString().c_str()),
                                   "CurrentVolume");
...
int setVolume(HTTPClient *http, IPAddress targetSonos, int setPoint) {
    if (http->begin(targetSonos.toString(), SONOS_PORT,
                    "/MediaRenderer/RenderingControl/Control")) {
        auto call = changeVolumeCall(setPoint);
        Serial.printf("POST: BODY %s\n", call.c_str());
        http->addHeader("Content-Type", "text/xml; charset=\"utf-8\"");
        http->addHeader(
            "SOAPACTION",
...
int volumeMute(HTTPClient *http, IPAddress targetSonos) {
    int currentVolume = getVolume(http, targetSonos);
    if (currentVolume < 0) {
        Serial.println("Couldn't get the current volume");
        return ENO_CANTCONNECT;
    ...
    if (currentVolume > 0) {
        ...
        return setVolume(http, targetSonos, 0);
    }
    // else currentVolume == 0 (muted)
    return setVolume(http, targetSonos, lastVolume);
}
